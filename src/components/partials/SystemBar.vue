<template>
  <div class="system-bar" :class="[styleClass, stylePlatform]">
    <div class="system-bar-resize-handle top"></div>
    <div class="system-bar-resize-handle right"></div>
    <div class="system-bar-resize-handle left"></div>
    <div v-if="platform === 'darwin'" class="system-bar-buttons-osx">
      <div
    ...
    <div class="system-bar-header">
      <div class="system-bar-icon" v-if="showIcon">
        <slot name="icon"></slot>
      </div>

      <div class="system-bar-name" v-if="showTitle">
        <slot name="title"></slot>
      </div>
    </div>

    <div class="system-bar-menu" v-if="platform !== 'darwin'">
      <div
        class="system-bar-menu-item"
    ...
    <div class="system-bar-buttons" v-if="platform !== 'darwin'">
      <button
        aria-label="minimize"
        title="Minimize"
        tabindex="-1"
...
export default {
  ...
  computed: {
    styleClass() {
      return `system-bar-style-${this.theme}`;
    },
    stylePlatform() {
      return `system-bar-platform-${this.platform}`;
    }
  }
};
</script>
<style lang="scss">
$system-bar-height: 28px;
.system-bar {
  position: relative;
  flex-grow: 0;
  flex-shrink: 0;
  ...
  .system-bar-icon ~ .system-bar-name {
    padding-left: 0;
  }
  &.system-bar-platform-darwin {
    .system-bar-header {
      width: 100%;
      text-align: center;
      position: absolute;
